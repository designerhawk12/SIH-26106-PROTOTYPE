...
function renderNode(node: MimeNode, depth: number, isLast: boolean, key: string): ReactElement[] {
  ...
  return [
    <div
      key={key}
      className="group relative flex items-baseline justify-between gap-4 py-1.5 pl-2 transition-colors hover:bg-surface-hover/50"
    >
      <span className="absolute inset-y-0 left-0 w-px bg-border group-hover:bg-accent/60" />
